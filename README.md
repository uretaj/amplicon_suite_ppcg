...
- `-o | --data_repo {repodir} `:  Directory where the required annotations for GRCh37 are stored. This creates the data_repo folder in the specified directory and sets the environment variable AA_DATA_REPO to repodir/data_repo if the folder does not exist in the directory or if the environment variable is not set.
