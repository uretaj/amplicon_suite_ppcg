#!/usr/bin/env python
#Original code from https://github.com/AmpliconSuite/AmpliconSuite-pipeline/
#Modified for PPCG by Jennifer Ureta - uretaj
...
parser.add_argument("-t", "--nthreads",
                    help="Number of threads to use in BWA and CNV calling", default=1)
...
#start of modification uretaj    
parser.add_argument("--scna_file", help="SCNA calls generated by Battenberg. Fields in the text file should"
                    " be in the Battenberg output format", default=None)
#end of modification uretaj    
...
#if args.sif and not args.sif.endswith("ppcg_ampliconsuite-pipeline.sif"):
#   if not args.sif.endswith("/"): args.sif+="/"
args.sif+="ppcg_amplicon_suite.sif"

#else:
#    args.sif = "ppcg_ampliconsuite-pipeline.sif"
...
#args.data_repo = os.path.realpath(args.data_repo)
...
#start of modification uretaj    
#convert SCNA format to bed file format needed by AA

if args.scna_file:
    args.cnv_bed = args.scna_file
    if not os.path.exists(args.output_directory+"/seeds"):
        print("CNV output directory did not exist. Made output directory.")
        os.makedirs(args.output_directory+"/seeds")
    scnadir, scnaname = os.path.split(args.scna_file)
    cnvdir = os.path.realpath(args.output_directory+"/seeds")
    out_file = args.output_directory+"/seeds/"+args.sample_name + "_seeds.bed"
    _chr = -1
    cnvname = args.sample_name + "_seeds.bed"
    with open(args.scna_file,"r",encoding="utf8") as in_scna, open(out_file,'wt') as out_bed:
        tsv_reader = csv.reader(in_scna,delimiter="\t")
        tsv_writer = csv.writer(out_bed,delimiter="\t")
        #skip header
        next(tsv_reader)
        for row in tsv_reader:
            ...
            frac1_A = float(row[11])
            nMaj1_A = float(row[9])
            nMin1_A = float(row[10])
            if row[14] != 'NA':
                frac2_A = float(row[14])
                nMaj2_A = float(row[12])
                nMin2_A = float(row[13])
            if frac1_A == 1:
                total_cn = nMaj1_A + nMin1_A
            else:
                total_cn =  frac1_A * (nMaj1_A + nMin1_A) + frac2_A * (nMaj2_A + nMin2_A)
            width  = end - start + 1                         
            if total_cn>=5  and total_cn!=1000.01 and width>=25000 and width<=10e6: 
                #merge adjacent intervals
                if _chr == chrom and _end + 1 == start:
                    _end = end
                    continue
                if _chr != -1:
                    row = [_chr,_start,_end]
...
argstring = "-t 1" + " --cngain " + str(args.cngain) + " --cnsize_min " + \
...
"""
    ...
    data_repo_d = args.data_repo + '/data_repo'
    no_data_repo = args.ref and not os.path.exists(data_repo_d +"/"+ args.ref)
    ...
    if no_data_repo and args.ref:
        ...
        #data_repo_d = args.data_repo + '/data_repo'
        outfile.write('echo downloading to {} ....\n'.format(data_repo_d))
...
"""                  
    ...
    """
    sing_string = "singularity exec --no-home --cleanenv --env-file " + env_outname + " --bind " \
                   + bamdir + ":/home/bam_dir --bind " + norm_bamdir + ":/home/norm_bam_dir " \
                  "--bind " + cnvdir + ":/home/bed_dir --bind " + args.output_directory + ":/home/output --bind " \
                  "$HOME/mosek:/home/mosek/ " + args.sif + " bash /home/run_paa_script.sh "
